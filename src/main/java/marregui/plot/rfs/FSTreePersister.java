...
/**
 * Transforms the tree structure represented by an FSEntry into XML
 *
 * @author marregui
 */
public class FSTreePersister {
    ...
    /**
     * @param root
     * @return The XML representing the FSEntry tree structure
     */
    ...
    private static String makeEntryTag(FSEntry entry, int indentLevel) {
        return indent(String.format(
                ...
                Boolean.valueOf(entry.isFolder),
                Long.valueOf(entry.size),
                Long.valueOf(entry.lastModified),
                entry.isFolder ? "" : "/"), indentLevel);
    }
}